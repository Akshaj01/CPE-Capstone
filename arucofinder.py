...
def main():
    ...
    while True:
        ...
        # create dictionary matching ids to distances from center
        distances = {}
        ...
        if ids is not None:
            for i in range(len(ids)):
                distances[ids[i]] = distance_from_cen(corners[i][0][0])
        
        # print the closest id to the center
        if len(distances) > 0:
...
def distance_from_cen(coords):
    return (coords[0] - 320)**2 + (coords[1] - 240)**2
